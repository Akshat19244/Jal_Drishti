...
class BeachesModule {
  ...
  async loadHeatmap(retry = 0) {
    ...
    try {
      ...
      const legend = document.getElementById('heatmapLegend');
      if (retry < 3) {
        if (legend) legend.innerHTML = `<span style="font-family:var(--mono);font-size:.62rem;color:var(--t3)">Retrying... (${retry+1}/3)</span>`;
        setTimeout(() => this.loadHeatmap(retry+1), (retry+1)*2000);
      } else {
        if (legend) legend.innerHTML = `<span style="font-family:var(--mono);font-size:.62rem;color:var(--crit)">
          ⚠ Heatmap unavailable.
          <button onclick="window.beachesModule.loadHeatmap(0)"
            style="margin-left:10px;font-size:.65rem;background:var(--sap);color:#fff;
            border:none;padding:3px 10px;border-radius:3px;cursor:pointer">Retry</button>
        </span>`;
      }
    }
  }
