...
class DashboardModule {
  ...
  async loadWQIDistribution(retry = 0) {
    try {
      ...
      if (retry < 3) setTimeout(() => this.loadWQIDistribution(retry+1), (retry+1)*2000);
    }
  }

  async loadTrendData(retry = 0) {
    try {
      ...
      if (retry < 2) setTimeout(() => this.loadTrendData(retry+1), (retry+1)*2000);
    }
  }

  async loadStateComparison(retry = 0) {
    try {
