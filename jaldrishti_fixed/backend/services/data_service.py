...
class DataService:
    ...
    def get_timeline_data(self, year, state='all'):
        ...
        grouped = data.groupby(['Station_Name', 'Latitude', 'Longitude']).agg({
            'DO': 'mean',
            'BOD': 'mean',
            'pH': 'mean',
            'Turbidity': 'mean',
    ...
    def get_namo_gangu_summary(self):
        ...
        state_stats = ganga.groupby('State').agg({
    ...
    def get_explorer_river_view(self):
        ...
        for basin_name in self.df['Basin'].dropna().unique():
            ...
            stations_grouped = basin_data.groupby('Station_Name').agg({
                'WQI': 'mean',
                'State': 'first',
            ...
            for _, row in stations_grouped.head(30).iterrows():
                ...
                trend = st_data.groupby('Year')['WQI'].mean().to_dict()
    ...
    def get_alerts(self, state='Gujarat', year=None):
        ...
        latest = data.sort_values('Date_Parsed').groupby('Station_Name').last().reset_index()
    ...
    def get_beaches(self, state='all'):
        ...
        grouped = beach_data.groupby(['Station_Name', 'Latitude', 'Longitude']).agg({
            'DO': 'mean', 'BOD': 'mean', 'pH': 'mean',
            'Turbidity': 'mean', 'Fecal_Coliform': 'mean',
            'EC': 'mean', 'WQI': 'mean',
            'Safety': lambda x: x.mode().iloc[0] if len(x.mode()) > 0 else 'Unknown',
    ...
    def get_chat_context(self, query):
        ...
        if 'pollut' in query_lower or 'worst' in query_lower or 'most' in query_lower:
            top = self.df.groupby('Basin')['WQI'].mean().nlargest(10)
...
def _safe_round(val, decimals=2):
    """Round to N decimals, handling NaN."""
